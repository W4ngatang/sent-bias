...
signif_results %>%
  ...
  mutate(model=factor(rename_model(model), levels=model_levels),
         test=factor(rename_test(test), test_levels)) %>%
  ggplot() +
  geom_tile(aes(x=model, y=test, fill=effect_size)) +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  scale_fill_gradient(low='white', high='black', limits=c(0,2))
ggsave('significant_word_effects.png', width=4, height=4)
```
...
signif_results %>%
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  scale_fill_gradient(low='white', high='black', limits=c(0,2))
ggsave('significant_sentence_effects.png', width=4, height=4)
```
