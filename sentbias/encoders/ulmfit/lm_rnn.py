...
def repackage_var(h):
    ...
    return tuple(repackage_var(v) for v in h) if type(h) in (tuple, list) else h.clone()
...
class RNN_Encoder(nn.Module):
    ...
    def __init__(self, ntoken, emb_sz, nhid, nlayers, pad_token, bidir=False,
        ...
        self.rnns = [nn.LSTM(emb_sz if layer == 0 else nhid, (nhid if layer != nlayers - 1 else emb_sz)//self.ndir,
                     1, bidirectional=bidir, dropout=dropouth) for layer in range(nlayers)]
    ...
    def forward(self, input):
        ...
        for layer, (rnn,drop) in enumerate(zip(self.rnns, self.dropouths)):
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                raw_output, new_h = rnn(raw_output, self.hidden[layer])
            new_hidden.append(new_h)
            raw_outputs.append(raw_output)
            if layer != self.nlayers - 1: raw_output = drop(raw_output)
            outputs.append(raw_output)

        self.hidden = repackage_var(new_hidden)
        return raw_outputs, outputs

    def one_hidden(self, layer):
        nh = (self.nhid if layer != self.nlayers - 1 else self.emb_sz)//self.ndir
