    ...
    for test in tests:
        ...
        if encs is None:
            ...
            for (i, k) in enumerate(('attr1', 'attr2', 'targ1', 'targ2')):
                ...
                sentences, sentences_lengths, sent2vec[i] = encode_sentences(
                    sentences, word_to_idx, MAX_SENTENCE_LENGTH)
            ...
            encsA = get_embedding_dictionary(
                model(
                    input[0],
                    encoded_sentences_lengths[0]),
                sent2vec[0])  # .data.numpy())
            encsB = get_embedding_dictionary(
                model(
                    input[1],
                    encoded_sentences_lengths[1]),
                sent2vec[1])  # .data.numpy())
            encsX = get_embedding_dictionary(
                model(
                    input[2],
                    encoded_sentences_lengths[2]),
                sent2vec[2])  # .data.numpy())
            encsY = get_embedding_dictionary(
                model(
                    input[3],
                    encoded_sentences_lengths[3]),
                sent2vec[3])  # .data.numpy())
